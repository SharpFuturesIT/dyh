...
class STM_LMS_Assignments {
	...
	public function __construct() {
		...
		add_filter(
			...
			function( $counter ) {
				return $counter + STM_LMS_Instructor_Assignments::total_pending_assignments() + STM_LMS_User_Assignment::my_assignments_statuses();
			}
		);
	...
	public function stm_lms_start_assignment() {
		if ( ! empty( $_GET['start_assignment'] ) && ! empty( $_GET['course_id'] ) ) {
			$user = STM_LMS_User::get_current_user();
			if ( empty( $user['id'] ) ) {
				wp_redirect( remove_query_arg( array( 'start_assignment', 'course_id' ), self::get_current_url() ) );
				die;
	...
	public static function _stm_lms_accept_draft_assignment( $draft_id = '', $course_id = '', $content = '' ) {
